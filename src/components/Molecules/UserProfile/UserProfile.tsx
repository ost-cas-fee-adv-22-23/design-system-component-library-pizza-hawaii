...
export const UserProfile: FC<TUserProfile> = ({ size = 'M', border, href, userName, userAvatar }) => {
			...
			return (
				<div className={[...baseStyle, 'overflow-hidden', sizeMap[size]].join(' ')}>
					<a href={href} className="transition duration-300 ease-in-out hover:scale-110">
						<Image src={userAvatar || noAvatar} alt={userName} width={imgMap[size]} height={imgMap[size]} />
					</a>
				</div>
			);
		} else {
	...
	return (
		<div className={[...baseStyle, 'overflow-hidden', sizeMap[size]].join(' ')}>
			<Image src={userAvatar} alt={userName} width={imgMap[size]} height={imgMap[size]} />
		</div>
	);
};
